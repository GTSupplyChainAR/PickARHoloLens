...
namespace PickAR.Navigation {
    ...
    public class WaypointCreator : MonoBehaviour {
        ...
        /// </summary>
        private void Start() {
            controllerInput = new ControllerInput(0, 0.05f);
        ...
        private void Update() {
            ...
            if (ModeSwitcher.isWaypointPlacement) {
                ...
                if ((Input.GetButtonDown(Buttons.CREATE)) || controllerInput.GetButtonDown(ControllerButton.X)) {
                    if (selectable == null) {
                        CreateWaypoint();
                    } else {
                        if (waypoint != null) {
				...
				if (InputSwitcher.isHoloLens && (Input.GetButtonDown(Buttons.SELECT)) || controllerInput.GetButtonDown(ControllerButton.Menu)) {
					waypoint.OnSelect();
				}

                if ((Input.GetButtonDown(Buttons.DRAG)) || controllerInput.GetButtonDown(ControllerButton.RightShoulder)) {
